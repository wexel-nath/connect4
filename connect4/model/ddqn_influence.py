...
class Model(ModelInterface):
    ...
    def train(self, history: History):
        ...
        last_q = 0
        for move in reversed(list(history.get_moves(self.player))):
            state = self._reshape_board(move.board)
            q = move.get_reward() + (self.gamma * last_q)
            last_q = q
    ...
    def predict(self, board: Board, player: int):
        return self.predict_with_board_array(board.board, board.get_valid_actions(), False)

    def predict_with_board_array(self, board, valid_actions, get_valid):
